...
def stitch(songs, countdown):
	...
	output = ffmpeg.output(processing, 'rpd_audio.mp3')
	ffmpeg.run(output)
...
def apply_fade(song: Song, fade_duration=2):
	filepath, fileext = os.path.splitext(song.filepath)
	fade_filepath = f'{filepath}-fade{fileext}'

	input = ffmpeg.input(song.filepath)
	processing = input.audio.filter('afade',type='in', curve='losi', duration=fade_duration)
	processing = processing.filter('afade', type='out', curve='losi',
																 start_time=song.get_fade_start_time(fade_duration),
...
def stitch_countdown(song: Song, countdown: Song):
	filepath, fileext = os.path.splitext(song.filepath)
